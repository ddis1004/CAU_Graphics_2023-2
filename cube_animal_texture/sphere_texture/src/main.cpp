...
Horse horse;
int vertSize = sizeof(horse.verts[0]) * horse.verts.size();
int normalSize = sizeof(horse.normals[0]) * horse.normals.size();
int texSize = sizeof(horse.texCoords[0]) * horse.texCoords.size();
...
void init()
{
	...
	viewMat = glm::lookAt(glm::vec3(0, 0, 1.5), glm::vec3(0, 0, 0), glm::vec3(0, 1, 0));
	...
	GLuint Texture = loadBMP_custom("Zebra.bmp");
